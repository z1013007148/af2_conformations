...
class MMSeqs2Runner:
    ...
    * "MMseqs2 enables sensitive protein sequence searching for the analysis
    ...
    def _submit(self) -> dict:
        ...
        res = requests.post(f"{ self.host_url }/ticket/msa", data=data, proxies=proxy)
    ...
    def _status(self, idx: str) -> dict:
        ...
        res = requests.get(f"{ self.host_url }/ticket/{ idx }", proxies=proxy)
    ...
    def _download(self, idx: str, path: str) -> NoReturn:
        ...
        res = requests.get(f"{ self.host_url }/result/download/{ idx }", proxies=proxy)
